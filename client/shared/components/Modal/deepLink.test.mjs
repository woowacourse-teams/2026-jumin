// buildDirectionsLinks가 지도별 앱·fallback URL을 제대로 만드는지 자동 확인하는 테스트
...
test('모든 지도 딥링크에 출발지와 목적지 좌표를 담는다', () => {
  const naverLinks = buildDirectionsLinks({ provider: 'NAVER', start, destination, appName: 'jumin' });
  const kakaoLinks = buildDirectionsLinks({ provider: 'KAKAO', start, destination, appName: 'jumin' });
  const tmapLinks = buildDirectionsLinks({ provider: 'TMAP', start, destination, appName: 'jumin' });
  const naver = new URL(naverLinks.appUrl);
  const kakao = new URL(kakaoLinks.appUrl);
  const tmap = new URL(tmapLinks.appUrl);
  ...
  for (const links of [naverLinks, kakaoLinks, tmapLinks]) {
    assert.match(links.webUrl, /^https:\/\//);
    assert.match(links.androidIntentUrl, /^intent:\/\//);
    assert.match(links.iosStoreUrl, /^https:\/\/apps\.apple\.com\//);
  }
});
