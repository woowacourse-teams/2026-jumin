...
검색 시 PostgreSQL의 `ST_DWithin`으로 직선거리 600m 이내 후보만 1차 조회한 뒤,
목적지와 주차장 좌표를 반경 100m 이내의 가장 가까운 보행 노드에 연결하고,
노드까지의 연결 거리를 포함해 `pgr_drivingDistance`로 도보거리를 계산합니다. 보행망에
연결되지 않는 주차장은 검색 결과에서 제외합니다. 보행망 자체가 아직 적재되지 않은
환경에서는 직선거리로 대체하지 않고 보행거리 계산 불가 오류를 반환합니다. 상세 조회에서
개별 경로를 찾지 못한 경우에는 `WALKING_ROUTE_NOT_FOUND`(404) 오류를 반환합니다.

현재 구현은 좌표를 가장 가까운 보행 링크의 임의 지점이 아니라 가장 가까운 보행 노드에
